...
const checkAuthRoutes = ['/dashboard', '/portal']
...
const routes = (server) => {
  ...
  server.get('/', (req, res, next) => req.cookies.lp_jwt ? res.redirect('/portal') : next(), handle)

  server.get('/logout', (req, res) => {
    console.log(getCookieDomainBy(req.headers.host))
    res.clearCookie('lp_jwt', { domain: getCookieDomainBy(req.headers.host) })
    return res.redirect('/')
  })

  server.get(/^\/.*\.(png|svg|css|ttf|jpg|ico|mp3)$/, cachingMiddleware, handle)
